                ...
                <div ref={imgRef} className={props.className} id={props.idName} >
                   <img  key={props.Id} src={isInView ? props.ImageName : ""} className="card-img-top" alt={props.ImageName}/>
                    <div className={`${props.cardbodyStyle ? props.cardbodyStyle : "card-body"}`} >
                        ...
                        {(propsCheck) ? <button type="button" class="btn btn-primary" onClick={() => props.AddToBasket({"Title": props.Title, "ID": props._id, "ImageName": props.ImageName, "URL": URL})} onMouseOver={()=>props.ShowBuyBtn({ID:props._id})}
                        
                        onMouseOut={()=>props.ShowPriceBtn({ID:props._id})}
