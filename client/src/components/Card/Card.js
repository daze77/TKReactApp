        ...
        <>
            <Col key={props._id} classstyle={props.classstyle} style={isInView ? {maxHeight:"auto"} : {height:"1000px"}} >
                <div ref={imgRef} className={props.className} id={props.idName} >
                   <img  key={props._id} src={isInView ? props.ImageName : ""} className="card-img-top" alt={props.ImageName}/>
                    <div className={`${props.cardbodyStyle ? props.cardbodyStyle : "card-body"}`} >
                        ...
                        {(propsCheck) ? <button type="button" className="btn btn-primary purchBtn" onClick={() => props.AddToBasket({"Title": props.Title, "ID": props._id, "ImageName": props.ImageName})} onMouseOver={()=>props.ShowBuyBtn({ID:props._id})} onMouseOut={()=>props.ShowPriceBtn({ID:props._id})}>{props.Price}
                        </button> : ''}
